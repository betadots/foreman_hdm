    ...
    initializer 'foreman_hdm.register_plugin', :before => :finisher_hook do |_app|
      Foreman::Plugin.register :foreman_hdm do
        ...
        security_block :foreman_hdm do
          permission :view_foreman_hdm, { :'foreman_hdm/example' => [:new_action],
                                                      :'react' => [:index] }
        end

        # Add a new role called 'Discovery' if it doesn't exist
        role 'ForemanPluginTemplate', [:view_foreman_hdm]
        ...
        widget 'foreman_hdm_widget', name: N_('Foreman plugin template widget'), sizex: 4, sizey: 1
      end
    end
